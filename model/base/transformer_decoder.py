...
class PositionEmbeddingSine1D(nn.Module):
    """
    A version of the position embedding for 1D sequences.
    """
    ...
    def forward(self, x, mask=None):
        ...
        if mask is None:
            mask = torch.zeros((x.size(0), x.size(2)), device=x.device, dtype=torch.bool)
...
class transformer_decoder(nn.Module):
  ...
  def __init__(self, args, num_queries, hidden_dim, dim_feedforward, nheads=4, num_layers=1, pre_norm=False):
    ...
    # self.pe_layer = PositionEmbeddingSine1D(N_steps, normalize=True)
  ...
  def forward(self, x, x_s, support_mask):
    ...
    # print(f"pos_x shape: {pos_x.shape}")  # Should be [1024, 2, 256]
    # print(f"src_x shape: {src_x.shape}")  # Should be [1024, 2, 256]
    # print(f"pos_x_s shape: {pos_x_s.shape}")  # Should be [1024, 2, 256]
    # print(f"src_x_s shape: {src_x_s.shape}")  # Should be [1024, 2, 256]
