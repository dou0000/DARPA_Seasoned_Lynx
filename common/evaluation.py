...
class Evaluator:
    r""" Computes true positives, false positives, and false negatives between prediction and ground-truth """
    @classmethod
    ...
    def classify_prediction(cls, pred_mask, batch):
        ...
        # compute true positives, false positives, and false negatives of each episode in a batch
        tp, fp, fn = [], [], []
        for _pred_mask, _gt_mask in zip(pred_mask, gt_mask):
            _gt_mask = _gt_mask.squeeze(0)
            _pred_mask = _pred_mask.squeeze(0)
            
            _tp = ((_pred_mask == 1) & (_gt_mask == 1)).sum().float()
            _fp = ((_pred_mask == 1) & (_gt_mask == 0)).sum().float()
            _fn = ((_pred_mask == 0) & (_gt_mask == 1)).sum().float()
